...
void workload::index_remove(INDEX *index, uint64_t key, int64_t part_id) {
  uint64_t pid = part_id;
  // if (part_id == -1) {
  //   pid = get_part_id(row);
  // }
  RC result = index->index_remove(key, pid);
  assert(result == RCOK);
}

void workload::initThread(const int __tid) {
  for (map<string, INDEX *>::iterator it = indexes.begin(); it != indexes.end();
       it++) {
