...
class txn_man {
 public:
  ...
  int index_range_query(INDEX* index, idx_key_t low, idx_key_t high,
                        idx_key_t* resultKeys, itemid_t** resultValues,
                        int part_id, bool countLen = false);
  ...
  void index_remove(INDEX* index, uint64_t key, int64_t part_id);
