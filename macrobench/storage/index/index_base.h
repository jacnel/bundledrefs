...
#define TRY_INIT_THREAD(tid)                                         \
  if (!IS_THREAD_INITIALIZED((tid))) {                               \
    ...
    if ((tid) >= MAX_TID_POW2)                                       \
      error(                                                         \
          "tid >= MAX_TID_POW2. too many threads created. increase " \
          "MAX_TID_POW2.");                                          \
...
class index_base {
 protected:
  ...
  unsigned long long numRemoves[MAX_TID_POW2 * PREFETCH_SIZE_WORDS];
  unsigned long long numReads[MAX_TID_POW2 * PREFETCH_SIZE_WORDS];
  //    #define INCREMENT_NUM_INSERTS(tid) { if
  //    ((++numInserts[(tid)*PREFETCH_SIZE_WORDS] % 100000) == 0)
  //    cout<<"tid="<<tid<<":
  //    numInserts="<<numInserts[(tid)*PREFETCH_SIZE_WORDS]<<endl; } #define
  //    INCREMENT_NUM_READS(tid) { if ((++numReads[(tid)*PREFETCH_SIZE_WORDS] %
  //    100000) == 0) cout<<"tid="<<tid<<":
  //    numReads="<<numReads[(tid)*PREFETCH_SIZE_WORDS]<<endl; }
#define INCREMENT_NUM_INSERTS(tid)
#define INCREMENT_NUM_REMOVES(tid)
...
#define PRINT_BUCKETS 30
 ...
 public:
  ...
  virtual RC init() {
    ...
    memset(numInserts, 0,
           MAX_TID_POW2 * PREFETCH_SIZE_WORDS * sizeof(unsigned long long));
    memset(numReads, 0,
           MAX_TID_POW2 * PREFETCH_SIZE_WORDS * sizeof(unsigned long long));
    for (int tid = 0; tid < MAX_TID_POW2; ++tid)
      IS_THREAD_INITIALIZED(tid) = false;
    return RCOK;
  };

  virtual RC init(uint64_t size) { return init(); };
  ...
  virtual RC index_read(KEY_TYPE key, VALUE_TYPE *item, int part_id = -1,
                        int thd_id = 0) = 0;
  virtual RC index_remove(KEY_TYPE key, int part_id = -1) = 0;
  virtual RC index_read(KEY_TYPE key, VALUE_TYPE *item, int part_id = -1) {
 ...
 public:
  inline void lock_key(KEY_TYPE key) { vwlock_acquire(PSLOCK(key)); }
  inline void unlock_key(KEY_TYPE key) { vwlock_release(PSLOCK(key)); }
