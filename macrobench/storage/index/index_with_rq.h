...
static Random
    rngs[MAX_TID_POW2 *
         PREFETCH_SIZE_WORDS];  // create per-thread random number generators
...
typedef record_manager<RECLAIMER_TYPE, ALLOCATOR_TYPE, POOL_TYPE, NODE_TYPE>
    RECORD_MANAGER_TYPE;
typedef bst<KEY_TYPE, VALUE_TYPE, less<KEY_TYPE>, RECORD_MANAGER_TYPE>
    INDEX_TYPE;
...
typedef record_manager<
    RECLAIMER_TYPE, ALLOCATOR_TYPE, POOL_TYPE, NODE_TYPE,
    SnapCollector<node_t<KEY_TYPE, VALUE_TYPE>, KEY_TYPE>,
    SnapCollector<node_t<KEY_TYPE, VALUE_TYPE>, KEY_TYPE>::NodeWrapper,
...
#elif (INDEX_STRUCT == IDX_SKIPLISTLOCK_RQ_UNSAFE)
#include "unsafe_skiplist_impl.h"
typedef node_t<KEY_TYPE, VALUE_TYPE> NODE_TYPE;
typedef bool DESCRIPTOR_TYPE;  // no descriptor
typedef record_manager<RECLAIMER_TYPE, ALLOCATOR_TYPE, POOL_TYPE, NODE_TYPE>
    RECORD_MANAGER_TYPE;
typedef unsafe_skiplist<KEY_TYPE, VALUE_TYPE, RECORD_MANAGER_TYPE> INDEX_TYPE;
#define INDEX_CONSTRUCTOR_ARGS                   \
  g_thread_cnt, numeric_limits<KEY_TYPE>::min(), \
      numeric_limits<KEY_TYPE>::max() - 1, __NO_VALUE, rngs
#define CALL_CALCULATE_INDEX_STATS_FOREACH_CHILD(x, depth)
...
#define INDEX_CONSTRUCTOR_ARGS \
  numeric_limits<KEY_TYPE>::max(), __NO_VALUE, g_thread_cnt
#define ISLEAF(x) ((x)->child[0]->val == NULL && (x)->child[1]->val == NULL)
#define CALL_CALCULATE_INDEX_STATS_FOREACH_CHILD(x, depth) \
  {                                                        \
    calculate_index_stats((x)->child[0]->val, (depth));         \
    calculate_index_stats((x)->child[1]->val, (depth));         \
  }
#define VALUES_ARRAY_TYPE VALUE_TYPE *
...
#define INDEX_CONSTRUCTOR_ARGS \
  g_thread_cnt, numeric_limits<KEY_TYPE>::max(), __NO_VALUE
#define CALL_CALCULATE_INDEX_STATS_FOREACH_CHILD(x, depth) \
  {                                                        \
    calculate_index_stats((x)->child[0], (depth));         \
    calculate_index_stats((x)->child[1], (depth));         \
...
typedef record_manager<RECLAIMER_TYPE, ALLOCATOR_TYPE, POOL_TYPE, NODE_TYPE>
    RECORD_MANAGER_TYPE;
typedef bslack<ABTREE_DEGREE, KEY_TYPE, less<KEY_TYPE>, RECORD_MANAGER_TYPE>
    INDEX_TYPE;
...
//       (and determine why they happen in orderline, and if it's ok)
  ...
  RC index_range_query(KEY_TYPE low, KEY_TYPE high, KEY_TYPE *resultKeys,
                       ...
                       int part_id = -1) {
    *numResults = index->rangeQuery(tid, low, high, resultKeys,
                                    (VALUES_ARRAY_TYPE)resultValues);
  ...
  size_t getNodeSize() { return sizeof(NODE_TYPE); }

  size_t getDescriptorSize() { return sizeof(DESCRIPTOR_TYPE); }
  ...
  void print_stats() {
    ...
    cout << "Avg depth: " << (num_nodes ? sum_nodes_depths / num_nodes : 0)
         << endl;
    cout << "Avg leaf depth: " << (num_leafs ? sum_leaf_depths / num_leafs : 0)
         << endl;
    for (int i = 0; i < 8; i++) {
      cout << "Alignment " << i * 8 << ": "
           << (alignment[i] / (double)num_nodes) * 100 << "%" << endl;
    }
  }
};
