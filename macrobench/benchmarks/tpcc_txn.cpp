...
RC tpcc_txn_man::run_payment(tpcc_query *query) {
  ...
  if (query->by_last_name) {
    ...
    c_since FROM customer WHERE c_w_id=:c_w_id AND c_d_id=:c_d_id AND
    c_last=:c_last ORDER BY c_first; EXEC SQL OPEN c_byname;
    +===========================================================================*/
    /*============================================================================+
    ...
    uint64_t key_low = custNPKey_ordered_by_cid(query->c_last, 0, query->c_d_id,
                                                query->c_w_id);
    uint64_t key_high = custNPKey_ordered_by_cid(query->c_last, g_cust_per_dist,
                                                 query->c_d_id, query->c_w_id);
    uint64_t resultKeys[key_high - key_low + 1];
    itemid_t *resultValues[key_high - key_low + 1];
    int numResults = index_range_query(index, key_low, key_high, resultKeys,
                                       resultValues, wh_to_part(c_w_id));
    assert(numResults > 0);
  ...
  /*======================================================================+
          EXEC SQL UPDATE customer SET c_balance = :c_balance, c_data =
  :c_new_data WHERE c_w_id = :c_w_id AND c_d_id = :c_d_id AND c_id = :c_id;
  +======================================================================*/
  ...
  if (strstr(c_credit, "BC")) {
    ...
    sprintf(c_new_data, "| %4lld %2lld %4lld %2lld %4lld $%7.2f",
            (long long)query->c_id, (long long)query->c_d_id,
            (long long)query->c_w_id, (long long)query->d_id,
...
+=======================================================*/
...
#ifndef READ_ONLY
  row_t *buf[15];  // can hold up to 15 orderline rows, which is the max allowed
                   // by tpc-c
  int bufsize = 0;
#endif
  for (UInt32 ol_number = 0; ol_number < ol_cnt; ol_number++) {
    ...
    int w_tax = 1, d_tax = 1;
    int64_t ol_amount =
        ol_quantity * i_price * (1 + w_tax + d_tax) * (1 - c_discount);
  ...
  index_insert(_wl->i_neworder, neworderKey(w_id, d_id, o_id), r_no,
               wh_to_part(w_id));
  for (int i = 0; i < bufsize; ++i) {
    index_insert(_wl->i_orderline, orderlineKey(w_id, d_id, o_id), buf[i],
                 wh_to_part(w_id));
    index_insert(_wl->i_orderline_wd, orderline_wdKey(w_id, d_id), buf[i],
                 wh_to_part(w_id));
  }
#endif
  return finish(rc);
...
RC tpcc_txn_man::run_order_status(tpcc_query *query) {
  /*	row_t * r_cust;
          if (query->by_last_name) {
                  ...
                  // EXEC SQL DECLARE c_name CURSOR FOR SELECT c_balance,
  c_first, c_middle, c_id
                  // FROM customer
                  // WHERE c_last=:c_last AND c_d_id=:d_id AND c_w_id=:w_id
  ORDER BY c_first;
                  // EXEC SQL OPEN c_name;
                  // if (namecnt%2) namecnt++; / / Locate midpoint customer for
  (n=0; n<namecnt/ 2; n++)
                  // {
                  ...
                  uint64_t key = custNPKey(query->c_last, query->c_d_id,
  query->c_w_id);
                  // XXX: the list is not sorted. But let's assume it's
  sorted...
                  // The performance won't be much different.
                  INDEX * index = _wl->i_customer_last;
                  uint64_t thd_id = get_thd_id();
                  itemid_t * item = index_read(index, key,
  wh_to_part(query->c_w_id)); int cnt = 0; itemid_t * it = item; itemid_t * mid
  = item; while (it != NULL) { cnt ++; it = it->next; if (cnt % 2 == 0) mid =
  mid->next;
                  }
                  r_cust = ((row_t *)mid->location);
          } else {
                  ...
                  uint64_t key = custKey(query->c_id, query->c_d_id,
  query->c_w_id); INDEX * index = _wl->i_customer_id; itemid_t * item =
  index_read(index, key, wh_to_part(query->c_w_id)); r_cust = (row_t *)
  item->location;
          }
  #if TPCC_ACCESS_ALL
          ...
          // EXEC SQL DECLARE c_line CURSOR FOR SELECT ol_i_id, ol_supply_w_id,
  ol_quantity,
  ...
  :ol_amount[i], :ol_delivery_d[i];
          ...
          while (item != NULL) {
                  ...
                  int64_t ol_i_id, ol_supply_w_id, ol_quantity, ol_amount,
  ol_delivery_d; r_orderline->get_value(OL_I_ID, ol_i_id);
...
// TODO concurrency for index related operations is not completely supported
// yet.
// In correct states may happen with the current code.

RC tpcc_txn_man::run_delivery(tpcc_query *query) {
  // XXX HACK if another delivery txn is running on this warehouse, simply
  // commit.
  // if (!ATOM_CAS(_wl->delivering[query->w_id], false, true)) return
  // finish(RCOK);

  for (int d_id = 1; d_id <= DIST_PER_WARE; d_id++) {
#ifdef INDEX_HAS_RQ
    // Read the most recent order ID.
    uint64_t key = distKey(d_id, query->w_id);
    itemid_t *item = index_read(_wl->i_district, key, wh_to_part(query->w_id));
    assert(item != NULL);
    row_t *r_dist = ((row_t *)item->location);
...
#endif
    key_high = neworderKey(query->w_id, d_id, o_id);
    uint64_t resultKeys[key_high - key_low + 1];
    itemid_t *resultValues[key_high - key_low + 1];
    int numResults =
        index_range_query(_wl->i_neworder, key_low, key_high, resultKeys,
                          resultValues, wh_to_part(query->w_id), true);
    if (numResults == 0) {
      continue;  // Maybe there is no new order to deliver.
    }
    item = resultValues[0];
    row_t *r_no = (row_t *)item->location;
    row_t *r_no_local = get_row(r_no, RD);
    if (r_no == NULL) {
      return finish(Abort);
    }

    int64_t no_o_id;
    r_no_local->get_value(NO_O_ID, no_o_id);

    // Deliver the order.
    key = orderPrimaryKey(query->w_id, d_id, no_o_id);
    item = index_read(_wl->i_order, key, wh_to_part(query->w_id));
    row_t *r_order = (row_t *)item->location;
    ...
    // item =
    //     index_read(_wl->i_orderline, orderlineKey(query->w_id, d_id,
    //     no_o_id),
    ...
    // Delete from index.
    key = neworderKey(query->w_id, d_id, no_o_id);
    index_remove(_wl->i_neworder, key, wh_to_part(query->w_id));
#else
    ...
    item = index_read(index, key, wh_to_part(query->w_id));
    row_t *r_order = (row_t *)item->location;
    row_t *r_order_local = get_row(r_neworder, WR);
    if (r_order_local == NULL) {
      return finish(Abort);
    }
    ...
    // Ignore updating customeer balance for now, and just remove line from
    // neworder index. key = custKey(o_c_id, d_id, query->w_id); item =
    // index_read(_wl->i_customer_id, key, wh_to_part(query->w_id)); row_t
    // *r_cust = (row_t *)item->location; double c_balance; uint64_t
    // c_delivery_cnt;
...
RC tpcc_txn_man::run_stock_level(tpcc_query *query) { return RCOK; }
