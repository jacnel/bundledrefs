 ...
 * programming. In Proceedings of the 25th Symposium on Operating Systems
 * Principles (SOSP '15)
 */
...
#define PERFORM_ALIGNMENT(obj_size)                                            \
  (obj_size + (ALIGN_NUMBER - (obj_size & ALIGN_MASK)))
#define ALIGN_OBJ_SIZE(obj_size)                                               \
  ((obj_size & ALIGN_MASK) ? PERFORM_ALIGNMENT(obj_size) : obj_size)

#define MOVE_PTR_FORWARD(p_obj, offset)                                        \
  ((intptr_t *)(((volatile unsigned char *)p_obj) + offset))
#define MOVE_PTR_BACK(p_obj, offset)                                           \
  ((intptr_t *)(((volatile unsigned char *)p_obj) - offset))

#define OBJ_HEADER_SIZE (sizeof(rlu_obj_header_t))
#define WS_OBJ_HEADER_SIZE (sizeof(rlu_ws_obj_header_t))

#define OBJ_TO_H(p_obj)                                                        \
  ((volatile rlu_obj_header_t *)MOVE_PTR_BACK(p_obj, OBJ_HEADER_SIZE))
#define H_TO_OBJ(p_h_obj)                                                      \
  ((volatile intptr_t *)MOVE_PTR_FORWARD(p_h_obj, OBJ_HEADER_SIZE))
#define OBJ_COPY_TO_WS_H(p_obj_copy)                                           \
  ((volatile rlu_ws_obj_header_t *)MOVE_PTR_BACK(                              \
      p_obj_copy, (OBJ_HEADER_SIZE + WS_OBJ_HEADER_SIZE)))
...
#define GET_THREAD_ID(p_obj)                                                   \
  (WS_GET_THREAD_ID(PTR_GET_WS_HEADER(GET_COPY(p_obj))))
...
#define TRY_CAS_PTR_OBJ_COPY(p_obj, new_ptr_obj_copy)                          \
  (CAS((volatile int64_t *)&(OBJ_TO_H(p_obj)->p_obj_copy), 0,                  \
       (int64_t)new_ptr_obj_copy) == 0)
...
#define CAS(addr, expected_value, new_value)                                   \
  __sync_val_compare_and_swap((addr), (expected_value), (new_value))
...
#define RLU_TRACE(self, fmt, ...)                                              \
  fprintf(stderr, "[%ld][%ld][%ld]:%s:%d:%s(): " fmt, self->uniq_id,           \
...
// GLOBALS
/////////////////////////////////////////////////////////////////////////////////////////
static volatile rlu_data_t g_rlu_data = {
    0,
};
...
static volatile rlu_thread_data_t *g_rlu_threads[RLU_MAX_THREADS] = {
    0,
};

static volatile long g_rlu_writer_locks[RLU_MAX_WRITER_LOCKS] = {
    0,
};

static volatile long g_rlu_array[RLU_CACHE_LINE_SIZE * 64] = {
    0,
};
...
static void rlu_add_writer_lock(rlu_thread_data_t *self, long writer_lock_id) {
  ...
  for (i = 0; i < n_locks; i++) {
    RLU_ASSERT(self->obj_write_set[self->ws_cur_id].writer_locks.ids[i] !=
               writer_lock_id);
  }

  self->obj_write_set[self->ws_cur_id].writer_locks.ids[n_locks] =
      writer_lock_id;
  self->obj_write_set[self->ws_cur_id].writer_locks.size++;

  RLU_ASSERT(self->obj_write_set[self->ws_cur_id].writer_locks.size <
             RLU_MAX_NESTED_WRITER_LOCKS);
}

static int rlu_try_acquire_writer_lock(rlu_thread_data_t *self,
                                       long writer_lock_id) {
  volatile long cur_lock;

  cur_lock = g_rlu_writer_locks[writer_lock_id];
  if (cur_lock == 0) {
    if (CAS(&g_rlu_writer_locks[writer_lock_id], 0, LOCK_ID(self->uniq_id)) ==
        0) {
      return 1;
    }
  }
...
static void rlu_release_writer_lock(rlu_thread_data_t *self,
                                    long writer_lock_id) {
...
static void rlu_release_writer_locks(rlu_thread_data_t *self, int ws_id) {
  ...
  for (i = 0; i < self->obj_write_set[ws_id].writer_locks.size; i++) {
    rlu_release_writer_lock(self,
                            self->obj_write_set[ws_id].writer_locks.ids[i]);
  }
}
...
static void rlu_reset_write_set(rlu_thread_data_t *self, long ws_counter) {
  ...
  self->obj_write_set[ws_id].p_cur =
      (intptr_t *)&(self->obj_write_set[ws_id].buffer[0]);

  rlu_reset_writer_locks(self, ws_id);
}

static intptr_t *rlu_add_ws_obj_header_to_write_set(rlu_thread_data_t *self,
                                                    intptr_t *p_obj,
                                                    obj_size_t obj_size) {
  intptr_t *p_cur;
...
static void rlu_add_obj_copy_to_write_set(rlu_thread_data_t *self,
                                          intptr_t *p_obj,
                                          obj_size_t obj_size) {
  intptr_t *p_cur;
  long cur_ws_size;
...
static void rlu_writeback_write_set(rlu_thread_data_t *self, long ws_counter) {
  ...
  for (i = 0; i < self->obj_write_set[ws_id].num_of_objs; i++) {
    ...
    memcpy((unsigned char *)p_obj_actual, (unsigned char *)p_obj_copy,
           obj_size);

    p_cur = MOVE_PTR_FORWARD(p_cur, ALIGN_OBJ_SIZE(obj_size));

    RLU_ASSERT_MSG(
        GET_THREAD_ID(p_obj_actual) == self->uniq_id, self,
...
static int rlu_writeback_write_sets_and_unlock(rlu_thread_data_t *self) {
  ...
  for (ws_counter = self->ws_head_counter; ws_counter < self->ws_wb_counter;
       ws_counter++) {
  ...
  ws_wb_num = 0;
  for (ws_counter = self->ws_wb_counter; ws_counter < self->ws_tail_counter;
       ws_counter++) {
...
static void rlu_process_free(rlu_thread_data_t *self) {
  ...
  for (i = 0; i < self->free_nodes_size; i++) {
    ...
    RLU_ASSERT_MSG(IS_UNLOCKED(p_obj), self,
                   "object is locked. p_obj = %p th_id = %ld\n", p_obj,
                   GET_THREAD_ID(p_obj));

    TRACE_3(self, "freeing: p_obj = %p, p_actual = %p\n", p_obj,
            (intptr_t *)OBJ_TO_H(p_obj));
...
static long rlu_wait_for_quiescence(rlu_thread_data_t *self,
                                    long version_limit) {
  int th_id;
  long iters;
  long cur_threads;
  ...
  for (th_id = 0; th_id < cur_threads; th_id++) {
    ...
    while (self->q_threads[th_id].is_wait) {
      ...
      if (self->q_threads[th_id].run_counter !=
          g_rlu_threads[th_id]->run_counter) {
        self->q_threads[th_id].is_wait = 0;
        break;
      }
      ...
      if (iters > Q_ITERS_LIMIT) {
        iters = 0;
        printf("[%ld] waiting for [%d] with: local_version = %ld , run_cnt = "
               "%ld\n",
               self->uniq_id, th_id, g_rlu_threads[th_id]->local_version,
               g_rlu_threads[th_id]->run_counter);
      }
...
static void rlu_commit_write_set(rlu_thread_data_t *self) {
  ...
  RLU_ASSERT(WS_INDEX(self->ws_tail_counter) !=
             WS_INDEX(self->ws_head_counter));
}
...
intptr_t *rlu_alloc(obj_size_t obj_size) {
  ...
  TRACE_3_GLOBAL("ptr=%p full_size=%zu ptr_to_obj=%p\n", ptr,
                 (OBJ_HEADER_SIZE + obj_size), (intptr_t *)H_TO_OBJ(p_obj_h));
...
intptr_t *rlu_deref_slow_path(rlu_thread_data_t *self, intptr_t *p_obj) {
  ...
  if ((self->is_steal) &&
    ...
    // => The other thread is going to wait for this thread to finish before
    // reusing the write-set log
    //    (to which p_obj_copy points)
    TRACE_1(
        self,
        "take copy from other writer th_id = %ld p_obj = %p p_obj_copy = %p\n",
...
int rlu_try_lock(rlu_thread_data_t *self, intptr_t **p_p_obj, size_t obj_size) {
  ...
  RLU_ASSERT_MSG(p_obj != NULL, self,
                 "[%ld] rlu_try_lock: tried to lock a NULL pointer\n",
                 self->writer_version);
  ...
  if (PTR_IS_LOCKED(p_obj_copy)) {
    ...
    if (th_id == self->uniq_id) {
      if (self->run_counter == WS_GET_RUN_COUNTER(p_ws_obj_h)) {
        ...
        // => return copy
        TRACE_2(self,
                "[%ld] already locked by this thread. p_obj = %p th_id = %ld\n",
      ...
      TRACE_1(self,
              "[%ld] already locked by another execution of this thread -> "
              "fail and sync. p_obj = %p th_id = %ld\n",
              self->local_version, p_obj, th_id);
    ...
    TRACE_1(self,
            "[%ld] already locked by another thread -> fail and request sync. "
            "p_obj = %p th_id = %ld\n",
            self->local_version, p_obj, th_id);
  ...
  RLU_ASSERT_MSG(GET_COPY(p_obj) == p_obj_copy, self,
                 "p_obj_copy = %p my_p_obj_copy = %p\n", GET_COPY(p_obj),
                 p_obj_copy);
...
void rlu_lock(rlu_thread_data_t *self, intptr_t **p_p_obj,
