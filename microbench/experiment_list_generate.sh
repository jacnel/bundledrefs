...
echo "Preparing experiment 0: IMPACT OF INCREASING UPDATE THREAD COUNT ON RQs FOR VARYING NUMBER OF RQ THREADS (rq size 100)"
rq=0
rqsize=100
nrqs="0 1 8"  # TODO: Make configurable.
for u in 50 ; do
for k in $ksizes ; do
...
temp=$(cat experiment_list.txt | grep 'lazylist')
temp=$(echo "${temp}" | grep 'bundle\|lockfree\|rwlock')
$(echo "${temp}" > experiment_list.txt)
