 ...
 *
 * This is an artifact to accompany the paper:
...
// TODO: use system clock (chrono::) to precisely calibrate CPU_FREQ_GHZ in a
// setup program (rather than having the user enter a specific GHZ number);
// then, get rid of chrono:: usage.
...
struct main_globals_t {
  ...
  Random rngs[MAX_TID_POW2 * PREFETCH_SIZE_WORDS];  // create per-thread random
                                                    // number generators (padded
...
#ifdef USE_DEBUGCOUNTERS
  debugCounter
      *keysum;  // key sum hashes for all threads (including for prefilling)
...
void *thread_prefill(void *_id) {
  ...
  while (!glob.done) {
    if (((++cnt) % OPS_BETWEEN_TIME_CHECKS) == 0) {
      ...
      if (chrono::duration_cast<chrono::milliseconds>(__endTime -
                                                      glob.startTime)
              .count() >= PREFILL_INTERVAL_MILLIS) {
        glob.done = true;
        __sync_synchronize();
        break;
      }
  ...
  __sync_bool_compare_and_swap(
      ...
      chrono::duration_cast<chrono::milliseconds>(__endTime - glob.startTime)
          .count());
  pthread_exit(NULL);
}

void prefill(DS_DECLARATION *ds) {
  chrono::time_point<chrono::high_resolution_clock> prefillStartTime =
      chrono::high_resolution_clock::now();

  const double PREFILL_THRESHOLD = 0.01;
  const int MAX_ATTEMPTS = 1000;
  const double expectedFullness =
      (INS + DEL ? INS / (double)(INS + DEL)
  ...
  for (attempts = 0; attempts < MAX_ATTEMPTS; ++attempts) {
    ...
    if (glob.running.load() > 0) {
      ...
      COUTATOMIC("Validation FAILURE: "
                 << glob.running.load()
                 << " non-responsive thread(s) [during prefill]" << endl);
      COUTATOMIC(endl);
      exit(-1);
    }
    /**
  ...
  const long totalSuccUpdates =
      counters->insertSuccess->getTotal() + counters->eraseSuccess->getTotal();
  COUTATOMIC("finished prefilling to size "
             << sz << " for expected size " << expectedSize
             << " keysum=" << glob.keysum->getTotal()
             << " dskeysum=" << ds->debugKeySum() << " dssize=" << ds->getSize()
             << ", performing " << totalSuccUpdates << " successful updates in "
             << (totalThreadsPrefillElapsedMillis / 1000.) /*(elapsed/1000.)*/
             << " seconds (total time " << (elapsed / 1000.) << "s)" << endl);
  CLEAR_COUNTERS;
#endif
#ifdef USE_GSTATS
  GSTATS_PRINT;
...
void *thread_timed(void *_id) {
  ...
  test_type *rqResultKeys =
      new test_type[RQSIZE + RQ_DEBUGGING_MAX_KEYS_PER_NODE];
  ...
  while (!glob.done) {
    if (((++cnt) % OPS_BETWEEN_TIME_CHECKS) == 0 ||
        (rq_cnt % RQS_BETWEEN_TIME_CHECKS) == 0) {
      chrono::time_point<chrono::high_resolution_clock> __endTime =
          chrono::high_resolution_clock::now();
      if (chrono::duration_cast<chrono::milliseconds>(__endTime -
                                                      glob.startTime)
              .count() >= abs(MILLIS_TO_RUN)) {
        __sync_synchronize();
        glob.done = true;
        __sync_synchronize();
        break;
...
void trial() {
  ...
  if (glob.running.load() > 0) {
    ...
    COUTATOMIC("Validation FAILURE: "
               << glob.running.load()
               << " non-terminating thread(s) [did we exhaust physical memory "
                  "and experience excessive slowdown due to swap mem?]"
               << endl);
    COUTATOMIC(endl);
    COUTATOMIC("elapsedMillis=" << glob.elapsedMillis
                                << " elapsedMillisNapping="
                                << glob.elapsedMillisNapping << endl);
  ...
  COUTATOMIC(((glob.elapsedMillis + glob.elapsedMillisNapping) / 1000.)
             << "s" << endl);
...
void printOutput() {
...
#ifdef USE_DEBUGCOUNTERS
  {
    ...
    if (threadsKeySum == dsKeySum) {
      cout << "Validation OK: threadsKeySum = " << threadsKeySum
  ...
  debugCounters *const counters = GET_COUNTERS;
  {
    const long long totalSearches =
        counters->findSuccess->getTotal() + counters->findFail->getTotal();
    const long long totalRQs =
        counters->rqSuccess->getTotal() + counters->rqFail->getTotal();
    const long long totalQueries = totalSearches + totalRQs;
    const long long totalUpdates =
        counters->insertSuccess->getTotal() + counters->insertFail->getTotal() +
  ...
  COUTATOMIC("napping milliseconds overtime : " << glob.elapsedMillisNapping
                                                << endl);
...
int main(int argc, char **argv) {
  // setup default args
  PREFILL = false;  // must be false, or else there's no way to specify no
                    // prefilling on the command line...
  MILLIS_TO_RUN = 1000;
  RQ_THREADS = 0;
  WORK_THREADS = 4;
  RQSIZE = 0;
...
// TODO: Find a way to keep strategy specific code out of main.
  ...
  if (!binding_isInjectiveMapping(TOTAL_THREADS, LOGICAL_PROCESSORS)) {
    cout << "ERROR: thread binding maps more than one thread to a single "
            "logical processor"
         << endl;
    exit(-1);
  }
...
#ifdef USE_DEBUGCOUNTERS
  // per-thread stats for prefilling and key-checksum validation of the data
  // structure
  ...
  cout << "garbage=" << glob.__garbage
       << endl;  // to prevent certain steps from being optimized out
#ifdef USE_DEBUGCOUNTERS
  delete glob.keysum;
