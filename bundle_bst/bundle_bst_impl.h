/**
 * Preliminary C++ implementation of unbalanced binary search tree using
 * LLX/SCX.
 *
...
bundle_bst_ns::Node<K, V>
  ...
  GSTATS_APPEND(tid, node_allocated_addresses,
                ((long long)newnode) % (1 << 12));
#endif
  return newnode;
}

template <class K, class V, class Compare, class RecManager>
long long bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::debugKeySum(
    Node<K, V> *node) {
  if (node == NULL) return 0;
  if ((void *)node->left == NULL) return (long long)node->key;
  return debugKeySum(node->left) + debugKeySum(node->right);
}

template <class K, class V, class Compare, class RecManager>
bool bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::validate(
    Node<K, V> *const node, const int currdepth, const int leafdepth) {
  return true;
}

template <class K, class V, class Compare, class RecManager>
bool bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::validate(
    const long long keysum, const bool checkkeysum) {
  return true;
}

template <class K, class V, class Compare, class RecManager>
inline int bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::size() {
  return computeSize((root->left)->left);
}

template <class K, class V, class Compare, class RecManager>
inline int bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::computeSize(
    Node<K, V> *const root) {
  if (root == NULL) return 0;
  if (root->left != NULL) {  // if internal node
    return computeSize(root->left) + computeSize(root->right);
  } else {  // if leaf
    return 1;
    //        printf(" %d", root->key);
  }
...
template <class K, class V, class Compare, class RecManager>
int bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::rangeQuery(
    const int tid, const K &lo, const K &hi, K *const resultKeys,
  ...
  Node<K, V> *prev = root;
  Node<K, V> *curr = root->left;
  bool is_left_child = false;
  while (curr != nullptr) {
    if (curr->key != this->NO_KEY && !cmp(curr->key, lo) &&
        !cmp(hi, curr->key)) {
      // Phase 2. Enter range
      if (is_left_child) {
        ...
        // curr = prev->right_bundle->getPtrByTimestamp(ts);
        curr = prev->right_bundle.getPtrByTimestamp(ts);
      }
      break;
    } else if (curr->key != this->NO_KEY && !cmp(hi, curr->key)) {
      // curr is larger than hi, go left
      prev = curr;
      curr = rqProvider->read_addr(tid, &curr->right);
      is_left_child = false;
    } else if (curr->key == this->NO_KEY || cmp(lo, curr->key)) {
      // curr is lower than lo, go right
      prev = curr;
      curr = rqProvider->read_addr(tid, &curr->left);
      is_left_child = true;
    } else {
      assert(false);
    }
...
template <class K, class V, class Compare, class RecManager>
const pair<V, bool> bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::find(
    const int tid, const K &key) {
  pair<V, bool> result;
  Node<K, V> *p;
  Node<K, V> *l;
...
template <class K, class V, class Compare, class RecManager>
const V bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::doInsert(
    const int tid, const K &key, const V &val, bool onlyIfAbsent) {
  V result = NO_VALUE;
  void *input[] = {(void *)&key, (void *)&val, (void *)&onlyIfAbsent};
  void *output[] = {(void *)&result};
...
template <class K, class V, class Compare, class RecManager>
const V bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::insertIfAbsent(
...
template <class K, class V, class Compare, class RecManager>
const pair<V, bool> bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::erase(
    const int tid, const K &key) {
  V result = NO_VALUE;
  void *input[] = {(void *)&key};
  void *output[] = {(void *)&result};
...
inline bool bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::
  ...
  TRACE COUTATOMICTID("updateInsert_search_llx_scx(tid=" << tid << ", key="
                                                         << key << ")" << endl);

  Node<K, V> *p = root, *l;
  l = rqProvider->read_addr(tid, &root->left);
  if (rqProvider->read_addr(tid, &l->left) !=
      NULL) {  // the tree contains some node besides sentinels...
    p = l;
    l = rqProvider->read_addr(
        tid,
        &l->left);  // note: l must have key infinity, and l->left must not.
    while (rqProvider->read_addr(tid, &l->left) != NULL) {
      p = l;
      if (cmp(key, p->key)) {
        l = rqProvider->read_addr(tid, &p->left);
  ...
  if (key == l->key) {
    if (onlyIfAbsent) {
      ...
      *result = val;  // for insertIfAbsent, we don't care about the particular
                      // value, just whether we inserted or not. so, we use val
                      // to signify not having inserted (and NO_VALUE to signify
                      // having inserted).
      return true;    // success
    }
    return false;  // Not implemented
    std::cerr << "In place updates are not supported by bundle_bst_impl "
                 "at this time..."
              << std::endl;
    exit(1);
  } else {
    ...
    if (l->key == NO_KEY || cmp(key, l->key)) {
      initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), l->key, l->value,
                     GET_ALLOCATED_NODE_PTR(tid, 0), l);
    } else {
      initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), key, val, l,
    ...
    *result = NO_VALUE;
    ...
    info->nodes[0] = p;
    info->nodes[1] = l;  // note: used as OLD value for CAS that changes p's
                         // child pointer (but is not frozen or marked)

    Node<K, V> *insertedNodes[] = {GET_ALLOCATED_NODE_PTR(tid, 0),
                                   GET_ALLOCATED_NODE_PTR(tid, 1), NULL};
    Node<K, V> *deletedNodes[] = {NULL};

    // Prepare the bundles
    //
    // Only two bundle entries are generated by this operation. First, the
    // parent of l (i.e., p) must be updated to point to the new router node
    // replacing l. Then, the router node must also be set. Both l and the
    // inserted node do not since they are both leaf nodes. Ex:
    //
    //         [p]                        [p]
    //         / \                       */ \
    //       [l]  ()          -->      [A1]  ()
    ...
    bool retval =
        scx(tid, info, (l == pleft ? &p->left : &p->right),
...
inline bool bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::
  ...
  TRACE COUTATOMICTID("updateErase_search_llx_scx(tid=" << tid << ", key="
                                                        << key << ")" << endl);
  ...
  p = l;
  l = rqProvider->read_addr(
      tid, &p->left);  // note: l must have key infinity, and l->left must not.
  while (rqProvider->read_addr(tid, &l->left) != NULL) {
    gp = p;
    p = l;
    if (cmp(key, p->key)) {
  ...
  if (key != l->key) {
    *result = NO_VALUE;
    ...
    if ((info->llxResults[0] = llx(tid, gp, &gpleft, &gpright)) == NULL)
      return false;
    if (p != gpleft && p != gpright) return false;
    if ((info->llxResults[1] = llx(tid, p, &pleft, &pright)) == NULL)
      return false;
    if (l != pleft && l != pright) return false;
    *result = l->value;
    // Read fields for the sibling s of l
    Node<K, V> *s = (l == pleft ? pright : pleft);
    if ((info->llxResults[2] = llx(tid, s, &sleft, &sright)) == NULL)
      return false;
    // Now, if the op. succeeds, all structure is guaranteed to be just as we
    // verified
    initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), s->key, s->value,
                   /*newWeight,*/ sleft, sright);
    info->numberOfNodes = 4;
    info->numberOfNodesToReclaim =
        3;  // reclaim p, s, l (reclamation starts at nodes[1])
...
template <class K, class V, class Compare, class RecManager>
bundle_bst_ns::Node<K, V>
    *bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::initializeNode(
        const int tid, Node<K, V> *const newnode, const K &key, const V &value,
...
// can have freed them--this is the case in this implementation). if this is
...
void bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::
  ...
  int highestIndexReached =
      (state == SCXRecord<K, V>::STATE_COMMITTED ? info->numberOfNodesToFreeze
                                                 : 0);
...
// you may call this only if each node in nodes is protected by a call to
// recmgr->protect
template <class K, class V, class Compare, class RecManager>
bool bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::scx(
    const int tid, ReclamationInfo<K, V> *const info,
    Node<K, V> *volatile
        *field,  // pointer to a "field pointer" that will be changed
    Node<K, V> *newNode, Node<K, V> *const *const insertedNodes,
    Node<K, V> *const *const deletedNodes) {
  TRACE COUTATOMICTID("scx(tid=" << tid << " type=" << info->type << ")"
                                 << endl);
  ...
  int i;
  for (i = 0; insertedNodes[i]; ++i)
    newdesc->c.insertedNodes[i] = insertedNodes[i];
  newdesc->c.insertedNodes[i] = NULL;
  for (i = 0; deletedNodes[i]; ++i)
    newdesc->c.deletedNodes[i] = deletedNodes[i];
  ...
  // note: writes equivalent to the following two are already done by
  // DESC1_NEW()
...
void bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::helpOther(
  ...
  // cout<<"sizeof(newrec)="<<sizeof(newrec)<<" computed
  // size="<<SCXRecord<K,V>::size<<endl;
  if (DESC1_SNAPSHOT(&newdesc, tagptr,
                     SCXRecord<K comma1 V>::
                         size /* sizeof(newrec) /*- sizeof(newrec.padding)*/)) {
...
int bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::help(
  ...
  // TODO: make SCX_WRITE_STATE into regular write for the owner of this
  // descriptor (might not work in general, but here we can prove that allfrozen
  // happens
  ...
  //  by the owner)
  for (int i = helpingOther; i < snap->c.numberOfNodesToFreeze;
       ++i) {  // freeze sub-tree
    ...
    bool successfulCAS = snap->c.nodes[i]->scxRecord.compare_exchange_strong(
        exp, tagptr);  // MEMBAR ON X86/64
    if (successfulCAS || exp == tagptr)
      continue;  // if node is already frozen for our operation

    // read mutable allFrozen field of descriptor
    bool succ;
    bool allFrozen =
        DESC1_READ_FIELD(succ, ptr->c.mutables, tagptr, MUTABLES_MASK_ALLFROZEN,
                         MUTABLES_OFFSET_ALLFROZEN);
    if (!succ) return SCXRecord<K, V>::STATE_ABORTED;

    int newState = (allFrozen) ? SCXRecord<K, V>::STATE_COMMITTED
                               : SCXRecord<K, V>::STATE_ABORTED;
    TRACE COUTATOMICTID("help return state "
                        << newState << " after failed freezing cas on nodes["
                        << i << "]" << endl);
    MUTABLES1_WRITE_FIELD(ptr->c.mutables, snap->c.mutables, newState,
                          MUTABLES_MASK_STATE, MUTABLES_OFFSET_STATE);
    return newState;
  }

  MUTABLES1_WRITE_BIT(ptr->c.mutables, snap->c.mutables,
                      MUTABLES_MASK_ALLFROZEN);
  for (int i = 1; i < snap->c.numberOfNodesToFreeze; ++i) {
    if (snap->c.scxRecordsSeen[i] == LLX_RETURN_IS_LEAF)
      continue;  // do not mark leaves
    snap->c.nodes[i]->marked.store(
        true, memory_order_relaxed);  // finalize all but first node
  }

  // CAS in the new sub-tree (update CAS)
  //    uintptr_t expected = (uintptr_t) snap->nodes[1];
  rqProvider->linearize_update_at_cas(tid, snap->c.field, snap->c.nodes[1],
                                      snap->c.newNode, snap->c.insertedNodes,
                                      snap->c.deletedNodes);
  ...
  MUTABLES1_WRITE_FIELD(ptr->c.mutables, snap->c.mutables,
                        SCXRecord<K comma1 V>::STATE_COMMITTED,
                        MUTABLES_MASK_STATE, MUTABLES_OFFSET_STATE);

  TRACE COUTATOMICTID("help return COMMITTED after performing update cas"
                      << endl);
...
template <class K, class V, class Compare, class RecManager>
void *bundle_bst_ns::bundle_bst<K, V, Compare, RecManager>::llx(
    const int tid, Node<K, V> *node, Node<K, V> **retLeft,
    Node<K, V> **retRight) {
  ...
  bool succ;
  int state =
      DESC1_READ_FIELD(succ, TAGPTR1_UNPACK_PTR(tagptr1)->c.mutables, tagptr1,
                       MUTABLES_MASK_STATE, MUTABLES_OFFSET_STATE);
  if (!succ) state = SCXRecord<K, V>::STATE_COMMITTED;
  // note: special treatment for alg in the case where the descriptor has
  // already been reallocated (impossible before the transformation, assuming
  // safe memory reclamation)

  SOFTWARE_BARRIER;  // prevent compiler from moving the read of marked before
                     // the read of state (no hw barrier needed on x86/64, since
                     // there is no read-read reordering)
  bool marked = node->marked.load(memory_order_relaxed);
  SOFTWARE_BARRIER;  // prevent compiler from moving the reads
                     // tagptr2=node->scxRecord or tagptr3=node->scxRecord
                     // before the read of marked. (no h/w barrier needed on
                     // x86/64 since there is no read-read reordering)
  if ((state & SCXRecord<K, V>::STATE_COMMITTED && !marked) ||
      state & SCXRecord<K, V>::STATE_ABORTED) {
    SOFTWARE_BARRIER;  // prevent compiler from moving the reads
                       // tagptr2=node->scxRecord or tagptr3=node->scxRecord
                       // before the read of marked. (no h/w barrier needed on
                       // x86/64 since there is no read-read reordering)
    ...
    *retRight = rqProvider->read_addr(tid, &node->right);
    if (*retLeft == NULL) {
      TRACE COUTATOMICTID("llx return2.a (tid="
                          << tid << " state=" << state << " marked=" << marked
                          << " key=" << node->key << ")\n");
      return LLX_RETURN_IS_LEAF;
    }
    SOFTWARE_BARRIER;  // prevent compiler from moving the read of
                       // node->scxRecord before the read of left or right
    tagptr_t tagptr2 = node->scxRecord.load(memory_order_relaxed);
    if (tagptr1 == tagptr2) {
      TRACE COUTATOMICTID("llx return2 (tid=" << tid << " state=" << state
                                              << " marked=" << marked
                                              << " key=" << node->key
                                              << " desc1=" << tagptr1 << ")\n");
      // on x86/64, we do not need any memory barrier here to prevent mutable
      // fields of node from being moved before our read of desc1, because the
      // hardware does not perform read-read reordering. on another platform, we
      // would need to ensure no read from after this point is reordered before
      // this point (technically, before the read that becomes desc1)...
    ...
    if (recmgr->shouldHelp()) {
      ...
      TRACE COUTATOMICTID("llx help 3 tid=" << tid << " tagptr3="
                                            << tagptrToString(tagptr3) << endl);
      helpOther(tid, tagptr3);
    }
  }
  TRACE COUTATOMICTID("llx return5 (tid=" << tid << " state=" << state
                                          << " marked=" << marked
                                          << " key=" << node->key << ")\n");
  return NULL;  // fail
}
