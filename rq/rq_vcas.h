...
#define TIMESTAMP_NOT_SET 0
 ...
 public:
  ...
  template <typename T>
  inline void write_vcas(const int tid, vcas_obj_t<T> volatile* const vcas_obj,
                         const T val) {
    initTS(vcas_obj);
    vcas_obj->val = val;
  }
  ...
  template <typename T>
  inline T read_vcas_unsafe(const int tid,
                            vcas_obj_t<T> volatile* const vcas_obj) {
    T val = vcas_obj->val;
    return val;
  }
...
#endif
      // count++;
      // std::cout << "ts: " << ts << ", node ts: " << head->ts << ", count: "
      // << count << endl;
      head = head->nextv;
    }
#if defined(VCAS_STATS)
  ...
  inline T linearize_update_at_write(const int tid,
    ...
    assert(false);                           // write_addr is disallowed
    write_vcas(tid, *lin_addr, lin_newval);  // original linearization point
  ...
  // Attempts to CAS in a new vcas_obj_
  template <typename T>
  inline bool cas_vcas(const int tid, vcas_obj_t<T>* volatile* const lin_vcas_obj,
    ...
    initTS(head);
    if (head->val != lin_oldval)
      res = false;
    else if (lin_newval == lin_oldval)
      res = true;
    else {
      vcas_obj_t<T>* new_head = new vcas_obj_t<T>(lin_newval, head);
      res = CAS(lin_vcas_obj, head, new_head);
      if (res) {
        initTS(new_head);
      } else {
        delete new_head;
  ...
  inline void traversal_end(const int tid, K* const rqResultKeys,
