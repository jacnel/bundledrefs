...
#ifdef DEBUG_RQ_PROVIDER_METRICS
        if (!foundDuringTraversal && nodeSource == NULL) threadData[tid].checkedFromLimbo += ds->getKeys(tid, node, outputKeys, outputValues);
#endif
...
#ifdef DEBUG_RQ_PROVIDER_METRICS
            if (!foundDuringTraversal && nodeSource == NULL) threadData[tid].insertedAfter += ds->getKeys(tid, node, outputKeys, outputValues);
#endif
...
#ifdef DEBUG_RQ_PROVIDER_METRICS
                if (!foundDuringTraversal && nodeSource == NULL) threadData[tid].deletedBefore += ds->getKeys(tid, node, outputKeys, outputValues);
#endif
...
#ifdef DEBUG_RQ_PROVIDER_METRICS
                    if (!foundDuringTraversal && nodeSource == NULL) threadData[tid].announcementRemovedWithoutDtime += ds->getKeys(tid, node, outputKeys, outputValues);
#endif
                    return 0;
                }
...
tryAddToRQ:
        ...
        if (cnt == 0) {
#ifdef DEBUG_RQ_PROVIDER_METRICS
            if (!foundDuringTraversal && nodeSource == NULL) ++threadData[tid].noKeys;
#endif
...
#ifdef DEBUG_RQ_PROVIDER_METRICS
                if (!foundDuringTraversal && nodeSource == NULL) ++threadData[tid].outOfRange;
#endif
                goto doNotAddToRQ;       // key is NOT in the desired range
            }
            if (threadData[tid].hashlist->contains(outputKeys[i])) {
#ifdef DEBUG_RQ_PROVIDER_METRICS
                if (!foundDuringTraversal && nodeSource == NULL) ++threadData[tid].alreadyInRQ;
#endif
...
public:
    ...
    void traversal_end(const int tid, K * const rqResultKeys, V * const rqResultValues, int * const startIndex, const K& lo, const K& hi) {
        ...
        GSTATS_ADD_IX(tid, visited_in_announcements, numVisitedInAnnouncements, threadData[tid].rq_lin_time);
