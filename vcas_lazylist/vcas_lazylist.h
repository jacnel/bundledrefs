...
class lazylist {
 private:
  ...
  RQProvider<K, V, node_t<K, V>, lazylist<K, V, RecManager>, RecManager, true,
             false> *const rqProvider;
 ...
 public:
  ...
  const K KEY_MAX;
  const V NO_VALUE;
  lazylist(int numProcesses, const K _KEY_MIN, const K _KEY_MAX,
           const V NO_VALUE);
  ~lazylist();
  ...
  long long getSizeInNodes() {
    long long size = 0;
    for (nodeptr curr = head->next->val; curr->key != KEY_MAX;
         curr = curr->next->val) {
      ++size;
    }
    return size;
