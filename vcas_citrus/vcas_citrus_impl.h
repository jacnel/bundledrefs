...
#include <pthread.h>
#include <utility>
#include "vcas_citrus.h"
#include "urcu.h"
#include "locks_impl.h"

namespace vcas_citrus {
using namespace std;
...
nodeptr citrustree<K, V, RecManager>::newNode(const int tid, K key, V value) {
  ...
  // Init vcas objects.
  nnode->child[0] = new vcas_obj_t<nodeptr>(nullptr, nullptr);
  rqProvider->write_vcas(tid, nnode->child[0], (nodeptr)NULL);
  nnode->child[1] = new vcas_obj_t<nodeptr>(nullptr, nullptr);
  rqProvider->write_vcas(tid, nnode->child[1], (nodeptr)NULL);
...
template <typename K, typename V, class RecManager>
citrustree<K, V, RecManager>::citrustree(const K bigger_than_max_key,
                                         const V _NO_VALUE,
                                         const int numProcesses)
    : recordmgr(new RecManager(numProcesses, SIGQUIT)),
      rqProvider(
          new RQProvider<K, V, node_t<K, V>, citrustree<K, V, RecManager>,
                         RecManager, LOGICAL_DELETION_USAGE, false>(
              numProcesses, this, recordmgr))
  ...
  rqProvider->write_vcas(tid, _root->child[0], _rootchild);
  ...
  // rqProvider->linearize_update_at_cas(tid, &root, (nodeptr) nullptr, _root,
  // insertedNodes,
  //                                     deletedNodes);
  root = _root;
#else
...
template <typename K, typename V, class RecManager>
const pair<V, bool> citrustree<K, V, RecManager>::find(const int tid,
                                                       const K& key) {
  recordmgr->leaveQuiescentState(tid, true);
  readLock();
  nodeptr curr = rqProvider->read_vcas(tid, root->child[0]);
  K ckey = curr->key;
  while (curr != NULL && ckey != key) {
    if (ckey > key) curr = rqProvider->read_vcas(tid, curr->child[0]);
    if (ckey < key) curr = rqProvider->read_vcas(tid, curr->child[1]);
...
template <typename K, typename V, class RecManager>
bool citrustree<K, V, RecManager>::validate(const int tid, nodeptr prev,
                                            int tag, nodeptr curr,
                                            int direction) {
  if (curr == NULL) {
    return (!prev->marked &&
            (rqProvider->read_vcas(tid, prev->child[direction]) == curr) &&
...
#define SEARCH                                           \
  prev = root;                                           \
  curr = rqProvider->read_vcas(tid, root->child[0]);     \
  direction = 0;                                         \
  ckey = curr->key;                                      \
  while (curr != NULL && ckey != key) {                  \
    prev = curr;                                         \
    if (ckey > key) {                                    \
      curr = rqProvider->read_vcas(tid, curr->child[0]); \
      direction = 0;                                     \
    }                                                    \
    if (ckey < key) {                                    \
      curr = rqProvider->read_vcas(tid, curr->child[1]); \
      direction = 1;                                     \
    }                                                    \
    if (curr != NULL) ckey = curr->key;                  \
  }
...
template <typename K, typename V, class RecManager>
const V citrustree<K, V, RecManager>::doInsert(const int tid, const K& key,
                                               const V& value,
                                               bool onlyIfAbsent) {
  nodeptr prev;
  nodeptr curr;
...
retry:
  ...
  if (curr != NULL) {
    if (onlyIfAbsent) {
      ...
      if (validate(tid, prev, tag, curr, direction)) {
        ...
        curr->marked = true;
        nodeptr oldnode = rqProvider->read_vcas(tid, prev->child[direction]);
        nodeptr nnode = newNode(tid, key, value);
        rqProvider->write_vcas(tid, nnode->child[0],
                               rqProvider->read_vcas(tid, curr->child[0]));
        rqProvider->write_vcas(tid, nnode->child[1],
                               rqProvider->read_vcas(tid, curr->child[1]));

        nodeptr insertedNodes[] = {nnode, NULL};
        nodeptr deletedNodes[] = {oldnode, NULL};
        rqProvider->linearize_update_at_cas(tid, &prev->child[direction],
                                            oldnode, nnode, insertedNodes,
                                            deletedNodes);
  ...
  if (validate(tid, prev, tag, curr, direction)) {
    ...
    nodeptr deletedNodes[] = {NULL};
    rqProvider->linearize_update_at_cas(tid, &prev->child[direction],
                                        (nodeptr) nullptr, nnode, insertedNodes,
                                        deletedNodes);
...
const V citrustree<K, V, RecManager>::insertIfAbsent(const int tid,
                                                     const K& key,
                                                     const V& val) {
...
template <class K, class V, class RecManager>
const V citrustree<K, V, RecManager>::insert(const int tid, const K& key,
                                             const V& val) {
  return doInsert(tid, key, val, false);
}

template <typename K, typename V, class RecManager>
const pair<V, bool> citrustree<K, V, RecManager>::erase(const int tid,
                                                        const K& key) {
  nodeptr prev;
  nodeptr curr;
  int direction;
...
retry:
  ...
  if (rqProvider->read_vcas(tid, curr->child[0]) == NULL) {
    ...
    rqProvider->linearize_update_at_cas(
        tid, &prev->child[direction], curr,
        rqProvider->read_vcas(tid, curr->child[1]), insertedNodes,
        deletedNodes);

    if (rqProvider->read_vcas(tid, prev->child[direction]) == NULL) {
      prev->tag[direction]++;
    }
  ...
  if (rqProvider->read_vcas(tid, curr->child[1]) == NULL) {
    curr->marked = true;
    ...
    rqProvider->linearize_update_at_cas(
        tid, &prev->child[direction], curr,
        rqProvider->read_vcas(tid, curr->child[0]), insertedNodes,
        deletedNodes);
  ...
  prevSucc = curr;
  succ = rqProvider->read_vcas(tid, curr->child[1]);
  next = rqProvider->read_vcas(tid, succ->child[0]);
  while (next != NULL) {
    prevSucc = succ;
    succ = next;
    next = rqProvider->read_vcas(tid, next->child[0]);
  }
  int succDirection = 1;
  if (prevSucc != curr) {
  ...
  if (validate(tid, prevSucc, 0, succ, succDirection) &&
    ...
    nodeptr curr_left = rqProvider->read_vcas(tid, curr->child[0]);
    nodeptr curr_right = rqProvider->read_vcas(tid, curr->child[1]);
    rqProvider->write_vcas(tid, nnode->child[0], curr_left);
    rqProvider->write_vcas(tid, nnode->child[1], curr_right);

    nodeptr insertedNodes[] = {nnode, NULL};
    nodeptr deletedNodes[] = {curr, succ, NULL};
    rqProvider->linearize_update_at_cas(tid, &prev->child[direction], curr,
                                        nnode, insertedNodes, deletedNodes);

    synchronize();

    // Remove succ. using cas_vcas since we have to add a new entry to the vcas
    // object.
    succ->marked = true;
    if (prevSucc == curr) {
      rqProvider->cas_vcas(tid, &nnode->child[1], curr_right,
                           rqProvider->read_vcas(tid, succ->child[1]));
      if (rqProvider->read_vcas(tid, nnode->child[1]) == NULL) {
        nnode->tag[1]++;
      }
    } else {
      rqProvider->cas_vcas(tid, &prevSucc->child[0],
                           rqProvider->read_vcas(tid, prevSucc->child[0]),
                           rqProvider->read_vcas(tid, succ->child[1]));
      if (rqProvider->read_vcas(tid, prevSucc->child[0]) == NULL) {
        prevSucc->tag[0]++;
      }
    }
...
template <typename K, typename V, class RecManager>
int citrustree<K, V, RecManager>::rangeQuery(const int tid, const K& lo,
  ...
  while (!stack.isEmpty()) {
    ...
    rqProvider->traversal_try_add(tid, node, resultKeys, resultValues, &size,
                                  lo, hi, ts);

    // if internal node, explore its children
    nodeptr left = rqProvider->read_vcas(tid, node->child[0]);
    nodeptr right = rqProvider->read_vcas(tid, node->child[1]);
...
long long citrustree<K, V, RecManager>::debugKeySum(nodeptr root) {
  ...
  return root->key + debugKeySum(root->child[0]->val) +
         debugKeySum(root->child[1]->val);
}

template <typename K, typename V, class RecManager>
long long citrustree<K, V, RecManager>::debugKeySum() {
  return debugKeySum(root->child[0]->val->child[0]->val);
}
}  // namespace vcas_citrus
#endif